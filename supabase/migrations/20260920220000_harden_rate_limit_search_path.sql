...
begin
  if p_key is null or pg_catalog.length(p_key) = 0 or p_limit <= 0 or p_window_seconds <= 0 then
  ...
  if bucket_reset_at <= now_at then
    ...
    return query select true, pg_catalog.greatest(0, p_limit - 1), 0;
    return;
  end if;

  if bucket_count >= p_limit then
    return query select false, 0,
      pg_catalog.greatest(1, pg_catalog.ceil(pg_catalog.extract(epoch from (bucket_reset_at - now_at)))::integer);
    return;
  end if;
  ...
  return query select true, pg_catalog.greatest(0, p_limit - bucket_count - 1), 0;
end;
$$;
